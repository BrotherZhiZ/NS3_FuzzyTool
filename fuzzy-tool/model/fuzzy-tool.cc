...
__attribute__((unused)) FILE *FuzzyTool::fisOpenFile(char *file, char *mode)
{
	FILE *fp/*, *fopen()*/;
...
__attribute__((unused))void *FuzzyTool::fisCalloc(int num_of_x, int size_of_x) 
{
	void *ptr;
	...
	if (num_of_x == 0) 
            ...
            if (ptr == NULL)
                fisError((char*)"Could not allocate memory in mxCalloc function call.");}
	...
	if (num_of_x == 0) 
            ...
            /* however we still need to check that memory was allocated successfully,
               exclude the case when num_of_x = 0, and if unsuccessful issue an error */
            if (ptr == NULL)
                fisError((char*)"Could not allocate memory in calloc function call.");
	}
#endif
...
__attribute__((unused))char **FuzzyTool::fisCreateMatrix(int row_n, int col_n, int element_size)
{
	char **matrix;
	int i;
	...
	if (matrix == NULL)
		fisError((char*)"Calloc error in fisCreateMatrix!");
	for (i = 0; i < row_n; i++) { 
...
__attribute__((unused)) void FuzzyTool::fisFreeMatrix(void **matrix, int row_n)
{
	int i;
	if (matrix != NULL) {
		for (i = 0; i < row_n; i++) {
...
__attribute__((unused)) DOUBLE**FuzzyTool::fisCopyMatrix(DOUBLE **source, int row_n, int col_n)
{
	DOUBLE **target;
	int i, j;
...
__attribute__((unused)) void FuzzyTool::fisPrintMatrix(DOUBLE **matrix, int row_n, int col_n)
{
	int i, j;
	for (i = 0; i < row_n; i++) {
		for (j = 0; j < col_n; j++)
...
__attribute__((unused)) void FuzzyTool::fisPrintArray(DOUBLE *array, int size)
{
	int i;
	for (i = 0; i < size; i++)
		PRINTF("%.3f ", array[i]);
...
__attribute__((unused)) void FuzzyTool::fisPause()
{
...
__attribute__((unused)) int FuzzyTool::fisGetMfParaN(char *mfType)
{
...
IO *FuzzyTool::fisBuildIoList(int node_n, int *mf_n)
{
	IO *io_list;
	int i, j;
...
void FuzzyTool::fisAssignMfPointer(FIS *fis)
{
	...
	struct command {
		...
		{ (char*)"trimf",	fisTriangleMf },
		{ (char*)"trapmf",	fisTrapezoidMf },
		{ (char*)"gaussmf",	fisGaussianMf },
		{ (char*)"gauss2mf",	fisGaussian2Mf },
		{ (char*)"sigmf",	fisSigmoidMf },
		{ (char*)"dsigmf",	fisDifferenceSigmoidMf },
		{ (char*)"psigmf",	fisProductSigmoidMf },
		{ (char*)"gbellmf",	fisGeneralizedBellMf },
		{ (char*)"smf",	fisSMf },
		{ (char*)"zmf",	fisZMf },
		{ (char*)"pimf",	fisPiMf },
		{ (char*)"linear",	NULL },
...
void FuzzyTool::fisAssignFunctionPointer(FIS *fis)
{
...
#ifdef MATLAB_MEX_FILE
	{
		...
		fisError((char*)"Legal andMethod: min, prod");
#endif
	}
...
#ifdef MATLAB_MEX_FILE
	{
		...
		fisError((char*)"Legal orMethod: max, probor");
#endif
	}
...
#ifdef MATLAB_MEX_FILE
	{
		...
		fisError((char*)"Legal impMethod: min, prod");
#endif
	}
...
#ifdef MATLAB_MEX_FILE
	{
		...
		fisError((char*)"Legal aggMethod: max, probor, sum");
#endif
	}
...
#ifdef MATLAB_MEX_FILE
	{
		...
		fisError((char*)"Legal defuzzification methods: centroid, bisector, mom, som, lom, wtaver, wtsum");
#endif
	}
}

#ifndef NO_PRINTF
__attribute__((unused)) void FuzzyTool::fisPrintData(FIS *fis)
{
	int i, j, k;

	if (fis == NULL)
		fisError((char*)"Given fis pointer is NULL, no data to print!");
...
__attribute__((unused)) void FuzzyTool::fisFreeMfList(MF *mf_list, int n)
{
	int i;
...
__attribute__((unused)) void FuzzyTool::fisFreeIoList(IO *io_list, int n)
{
	int i;
	for (i = 0; i < n; i++) {
...
void FuzzyTool::fisFreeFisNode(FIS *fis)
{
	if (fis == NULL)
		return;
...
void FuzzyTool::fisComputeOutputMfValueArray(FIS *fis, int numofpoints)
{
	int i, j, k;
	DOUBLE x, lx, ux, dx;
	MF *mf_node;
...
__attribute__((unused)) void FuzzyTool::fisGetString2(char *target, DOUBLE *array, int max_leng)
{
	int i;
	int actual_leng;
...
*/
void FuzzyTool::fisCheckDataStructure(FIS *fis)
{
	int i, j, mf_index;
	int found;
...
__attribute__((unused)) void FuzzyTool::fisBuildFisNode(FIS *fis, DOUBLE **fismatrix, int col_n, int numofpoints)
{
	int i, j, k;
	...
	/* get Sugeno output equation parameters */
		...
		PRINTF("fis->type = %s\n", fis->type);
		fisError((char*)"Unknown fis type!");
	}
...
__attribute__((unused)) void FuzzyTool::fisLoadParameter(FIS *fis, DOUBLE **fismatrix, int numofpoints)
{
	int start;
	int i, j, k;
...
__attribute__((unused)) void FuzzyTool::fisLoadParameter1(FIS *fis, DOUBLE *para_array, int numofpoints)
{
	int start = 0;
	int paraN;
	int i, j, k;
...
__attribute__((unused)) FIS *FuzzyTool::fisMatchHandle(FIS *head, int handle)
{
	FIS *p;
	...
	if (p == NULL) {
		...
		fisError((char*)"Cannot find an FIS with this handle.");
	}
	return(p);
}
...
__attribute__((unused)) FIS *FuzzyTool::fisMatchName(FIS *head, char *name)
{
...
__attribute__((unused)) int FuzzyTool::fisFindMaxHandle(FIS *head)
{
	FIS *p;
	int max_handle = 0;
...
__attribute__((unused)) void FuzzyTool::fisComputeInputMfValue(FIS *fis)
{
	int i, j;
	MF *mf_node;
...
__attribute__((unused)) void FuzzyTool::fisComputeTskRuleOutput(FIS *fis)
{
	int i, j, k;
	DOUBLE out;
	MF *mf_node;
...
__attribute__((unused)) void FuzzyTool::fisComputeFiringStrength(FIS *fis)
{
	...
	for (i = 0; i < fis->rule_n; i++) {
		if (fis->and_or[i] == 1) {	/* AND premise */
			...
			if (!fis->userDefinedAnd)
				out = fisArrayOperation(fis->mfs_of_rule, fis->in_n, fis->andFcn);
			else {
#ifdef MATLAB_MEX_FILE
...
__attribute__((unused)) DOUBLE FuzzyTool::fisFinalOutputMf(FIS *fis, int m, int n)
{
	int i, which_mf;
	DOUBLE mf_value, out;
...
/* (for Mamdani's model only */
__attribute__((unused)) void FuzzyTool::fisFinalOutputMf2(FIS *fis, int m, DOUBLE *aggMF, int numofpoints)
{
	int i, j, which_mf;
...
void FuzzyTool::fisEvaluate(FIS *fis, int numofpoints)
{
	DOUBLE out = 0;
	...
	for (i = 0; i < fis->out_n; i++) {
		...
		if (strcmp(fis->defuzzMethod, "wtaver") == 0)
			...
			fisError((char*)"Legal methods: wtaver, wtsum");
		}
	}
	}
...
__attribute__((unused)) void FuzzyTool::getFisOutput(DOUBLE *input, FIS *fis, DOUBLE *output)
{
	int i;
...
__attribute__((unused)) char *FuzzyTool::getNextLine(char *buf, FILE *fp)
{
    char  *returned_value;
    int i, j;
    ...
    if ((buf[0] == '%') || (buf[0] == '\n') || compareString((char*)"Version",buf) == 1 )
...
/* find number x in "******=x" */
__attribute__((unused)) DOUBLE FuzzyTool::getNumber(char *buf, FILE *fp)
{
	int tmp;
	char string[STR_LEN];
	DOUBLE num;
	...
	if (getNextLine(buf, fp) == NULL)
		fisError((char*)"getNumber: Incomplete FIS file!");

	tmp = sscanf(buf, " %[^=] = %lf ", string, &num);
	if (tmp != 2) {
		PRINTF("Error format in FIS file when parsing\n");
		PRINTF("\"%s\"\n", buf);
		fisError((char*)"Error in getNumber().");
	}
	/*
...
/* find string x in "*******='x'" */
__attribute__((unused)) void FuzzyTool::getString(char *buf, FILE *fp, DOUBLE *array)
{
	int i;
	...
	if (getNextLine(buf, fp) == NULL)
		fisError((char*)"getString: Incomplete FIS file!");
	
	tmp = sscanf(buf, " %[^'] '%[^']' ", string1, string2);
	if (tmp != 2) {
		PRINTF("Error format in FIS file when parsing\n");
		PRINTF("\"%s\"\n", buf);
		fisError((char*)"Error in getString().");
	}
...
__attribute__((unused)) int FuzzyTool::getArray(char *string, DOUBLE *array)
{
	int i;
	int start, end, index;
	char tmp[STR_LEN];
...
/*�ƺ����������������ȡ�ļ�FIS�ļ�*/
__attribute__((unused)) void FuzzyTool::getMfN(char *filename, int in_n, DOUBLE *in_mf_n, int out_n, DOUBLE *out_mf_n)
{
	int i, tmp;
	char buf[STR_LEN];
	FILE *fp = fisOpenFile(filename, (char*)"r");

	for (i = 0; i < in_n+out_n; i++) {
		while (1) {
			if (getNextLine(buf, fp) == NULL)
				fisError((char*)"Not enough NumMFs in FIS file!");
...
__attribute__((unused)) DOUBLE **FuzzyTool::returnEmptyFismatrix(char *filename, int *row_n_p, int *col_n_p)
{
	...
	/* find the row_n */
	fp = fisOpenFile(filename, (char*)"r");
	/* find in_n */
	while (1) {
		if (getNextLine(buf, fp) == NULL)
			fisError((char*)"Cannot find NumInputs in FIS file!");
	...
	while (1) {
		if (getNextLine(buf, fp) == NULL)
			fisError((char*)"Cannot find NumOutputs in FIS file!");
	...
	while (1) {
		if (getNextLine(buf, fp) == NULL)
			fisError((char*)"Cannot find NumRules in FIS file!");
	...
	while (1) {
		if (getNextLine(buf, fp) == NULL)
			fisError((char*)"Cannot find FIS Name in FIS file!");
	...
	while (1) {
		if (getNextLine(buf, fp) == NULL)
			fisError((char*)"Cannot find FIS Type in FIS file!");
...
__attribute__((unused)) DOUBLE **FuzzyTool::returnFismatrix(char *fis_file, int *row_n_p, int *col_n_p)
{
	int i, j, k;
	FILE *fp;
	char buf[STR_LEN];
	...
	fp = fisOpenFile(fis_file, (char*)"r");
	/* looping till it finds "[System]" */
	while (1) {
		if (getNextLine(buf, fp) == NULL)
			fisError((char*)"Cannot find [System] in FIS file!");
	...
	*/
	...
	while (1) {
		if (getNextLine(buf, fp) == NULL)
			fisError((char*)"Cannot find the first Name in FIS file!");
	...
	for (i = 0; i < in_n+out_n; i++) {
		while (1) {
			if (getNextLine(buf, fp) == NULL)
				fisError((char*)"Not enough Name in FIS file!");
			if (sscanf(buf, " Name = '%[^']' ", str1) == 1)
				break;
		}
	...
	for (i = 0; i < in_n+out_n; i++) {
		while (1) {
			if (getNextLine(buf, fp) == NULL)
				fisError((char*)"Not enough Range in FIS file!");
			if (sscanf(buf, " Range = [ %[^]] ", str1) == 1)
				break;
		}
		if (getArray(str1, fismatrix[now++]) != 2)
			fisError((char*)"Error in parsing I/O ranges.");
	}
	fclose(fp);
	...
	for (i = 0; i < in_n+out_n; i++) {
		...
		for (j = 0; j < mf_n; j++) {
			while (1) {
				if (getNextLine(buf, fp) == NULL)
					fisError((char*)"Not enough MF Labels in FIS file!");
				if (sscanf(buf, " %[^']'%[^']' : '%[^']' , [ %[^]] ",
	...
	for (i = 0; i < in_n+out_n; i++) {
		...
		for (j = 0; j < mf_n; j++) {
			while (1) {
				if (getNextLine(buf, fp) == NULL)
					fisError((char*)"Not enough MF types in FIS file!");
				if (sscanf(buf, " %[^']'%[^']' : '%[^']' , [ %[^]] ",
					str1, str2, str3, str4) == 4)
					break;
			}
	...
	for (i = 0; i < in_n+out_n; i++) {
		...
		for (j = 0; j < mf_n; j++) {
			while (1) {
				if (getNextLine(buf, fp) == NULL)
					fisError((char*)"Not enough MF parameters in FIS file!");
				if (sscanf(buf, " %[^']'%[^']' : '%[^']' , [ %[^]] ",
					str1, str2, str3, str4) == 4) {
					/*
					printf("%s\n", buf);
			...
			if (i < in_n) {
				if (getArray(str4, fismatrix[now]) > MF_PARA_N) {
					...
					*/
					fisError((char*)"Error in parsing input MF parameters.");
				}
			} else {
				if (!strcmp(fisType, "mamdani")) {
					if (getArray(str4, fismatrix[now]) > MF_PARA_N) {
						fisError((char*)"Error in parsing output MF parameters.");
					}
				} else {	/* sugeno system */
					int tmp = getArray(str4, fismatrix[now]);
					if (!strcmp(str3, "constant")){
						if (tmp != 1)
							fisError((char*)"Zero-order Sugeno system does not has the right number of output MF parameters.");
						...
						if (tmp != in_n+1)
							fisError((char*)"First-order Sugeno system does not has the right number of output MF parameters.");
					} else {
						fisError((char*)"Unknown output MF type for Sugeno system.");
					}
				}
			}
			now++;
	...
	while (1) {
		if (getNextLine(buf, fp) == NULL)
			fisError((char*)"Cannot find [Rules] in FIS file!");
		if (!strcmp(buf, "[Rules]")) /* found it! */
			break;
	}
	for (i = 0; i < rule_n; i++) {
		if (getNextLine(buf, fp) == NULL)
			fisError((char*)"Not enough rule list in FIS file!");
		/* get rid of ",", "(" and ")" */
		...
		if (getArray(buf, fismatrix[now++]) != in_n + out_n + 2) {
			...
			*/
			fisError((char*)"Error in parsing rule list!");
		}
	}
	fclose(fp);
...
/* return data matrix */
DOUBLE **FuzzyTool::returnDataMatrix(char *filename, int *row_n_p, int *col_n_p)
{
	DOUBLE **datamatrix;
	...
	for (i = 0; i < row_n; i++) {
		for (j = 0; j < col_n; j++) {
			if (fscanf(fp, "%lf", &num1) != EOF)
				...
				fisError((char*)"Not enough data in data file!");
		}
	}
	fclose(fp);
